...
func (f FilterFn) Filter(nodes []*kyaml.RNode) ([]*kyaml.RNode, error) {
	return f(nodes)
}
...
func WithFilter(value FilterFn) RenderOptsFn {
	return func(opts *renderOpts) {
		opts.plugins = append(opts.plugins, filter(value))
	}
}
...
func WithFilters(values ...FilterFn) RenderOptsFn {
	return func(opts *renderOpts) {
		for i := range values {
			opts.plugins = append(opts.plugins, filter(values[i]))
		}
	}
}
