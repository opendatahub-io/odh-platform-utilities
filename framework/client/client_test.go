...
func TestClient_List_WithMatchingLabels(t *testing.T) {
	t.Parallel()
	g := NewWithT(t)
	...
	wrappedClient := uclient.New(newFakeClient(cm1, cm2))

	result := &corev1.ConfigMapList{}
	err := wrappedClient.List(t.Context(), result,
	...
	g.Expect(err).ShouldNot(HaveOccurred())
	g.Expect(result.Items).Should(HaveLen(1))
	g.Expect(result.Items[0].Name).Should(Equal("labeled-cm"))
}
