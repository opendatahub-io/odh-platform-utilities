...
// WithClusterTypes filtering. Return true to skip the check; the condition
// type is initialized as True and an Info-level log is emitted. Return an error
...
func skippedByClusterType(pc *PreCondition, clusterType cluster.ClusterType) bool {
	return clusterType != "" && len(pc.clusterTypes) > 0 && !slices.Contains(pc.clusterTypes, clusterType)
}
...
func evalPreCondition(
	...
	if skippedByClusterType(pc, clusterType) {
