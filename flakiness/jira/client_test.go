...
func newTestClient(t *testing.T, srv *httptest.Server, opts ...func(*jira.Config)) *jira.Client {
	t.Helper()
...
func TestClientCreateBug(t *testing.T) {
	...
	t.Run("includes component and labels when configured", func(t *testing.T) {
		...
		_, err := newTestClient(t, srv, func(cfg *jira.Config) {
			cfg.Component = "testing"
			cfg.Labels = []string{"flaky-test", "quarantine"}
		}).CreateBug(context.Background(), entry)
		require.NoError(t, err)
	...
	t.Run("description contains flakiness details", func(t *testing.T) {
		...
		jira.SetClientClock(client, func() time.Time { return fixedNow })
		...
		assert.Contains(t, text, "2026-07-15")  // last failed (from entry)
		assert.Contains(t, text, "2026-01-10")  // quarantined (fixed clock)
		assert.Contains(t, text, "2026-02-09")  // re-enable = 2026-01-10 + 30d
		assert.Greater(t, len(paragraphs), 1)   // each field is its own paragraph
	})

	t.Run("omits optional fields when entry is minimal", func(t *testing.T) {
		t.Parallel()
		...
		srv := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			_ = json.NewDecoder(r.Body).Decode(&captured)
			w.WriteHeader(http.StatusCreated)
			_, _ = w.Write([]byte(`{"key":"TEST-125"}`))
		}))
		defer srv.Close()

		minimal := flakiness.QuarantineEntry{
			Name:       "TestMinimal",
			FlakeRate:  0.1,
			TotalRuns:  10,
			FailedRuns: 1,
		...
		client := newTestClient(t, srv, func(cfg *jira.Config) {
			cfg.QuarantineDuration = 0
		})
		jira.SetClientClock(client, func() time.Time { return time.Date(2026, 1, 10, 0, 0, 0, 0, time.UTC) })

		_, err := client.CreateBug(context.Background(), minimal)
		require.NoError(t, err)
		...
		assert.Contains(t, text, "TestMinimal")
		assert.NotContains(t, text, "Suite: ")
		assert.NotContains(t, text, "Job: ")
		assert.NotContains(t, text, "Last failed: ")
		assert.NotContains(t, text, "Expected re-enable: ")
	})
